...
if(isset($_POST['gouserauth'])){
    ...
    if(mysqli_num_rows($query)==1){
        ...
        if(mysqli_num_rows($query)==1) {
            $query = $conn->query("SELECT * FROM users WHERE email = '$email' AND pass = '$pass' AND confirm = '1'");
            if(mysqli_num_rows($query)==1) {
...
?>
<div id="myModalHauth" class="modal fade" role="dialog">
            ...
            <div class="modal-body">
                <div class="container">
                    <form action="#" method="post">
                        ...
                        <button type="submit" class="btn btn-success w-100" name="gouserauth">Вход</button>
                    </form>
                </div>
            </div>
        </div>
